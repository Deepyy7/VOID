...
// ── Parameter IDs (match JS PARAM_IDS) ───────────────────────
...
juce::AudioProcessorValueTreeState::ParameterLayout VOIDProcessor::createParameterLayout()
{
    ...
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_PREDELAY, "Pre-Delay", 0.0f, 750.0f, 234.0f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_DIFFUSION, "Diffusion", 0.0f, 1.0f, 0.60f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_SIZE, "Size", 5.0f, 100.0f, 54.5f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_MODDEPTH, "Mod Depth", 0.0f, 100.0f, 24.0f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_MODRATE, "Mod Rate", 0.1f, 5.0f, 1.27f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_ATTACK, "Attack", 0.0f, 1.0f, 1.0f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_HIGHCUT, "High Cut", 1000.0f, 20000.0f, 11092.0f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_HIGHMULT, "Hi Mult", 0.1f, 2.0f, 0.1f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_HIGHXOVER, "Hi X-Over", 500.0f, 8000.0f, 6000.0f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_LOWCUT, "Low Cut", 20.0f, 500.0f, 287.0f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_LOWMULT, "Lo Mult", 0.1f, 2.0f, 0.26f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_MIX, "Mix", 0.0f, 100.0f, 42.0f));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        ID_DECAY, "Decay", 0.3f, 20.0f, 5.1f));
...
// ── Map our params → JUCE Reverb params ───────────────────────
void VOIDProcessor::updateReverbParams()
{
    ...
    const float decay   = apvts.getRawParameterValue (ID_DECAY)->load()   / 20.0f; // normalise
...
// ── Process ───────────────────────────────────────────────────
void VOIDProcessor::processBlock (juce::AudioBuffer<float>& buffer,
                                   juce::MidiBuffer&)
{
